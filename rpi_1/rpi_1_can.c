...
int moreEight(char *buf, int len) {
    // 모든 packet에 rpc 고유 id, last packet 식별자, lineNum 붙여서 보내기 
        ...
        //ex) helloworld~라면 211helloworld~로 넘어와서 len이 14 
        //201hello, 201world, 211~ 이렇게 세 개의 packet 전달
        //전체 packet은 2, last는 4로 계산됨. 
        ...
        for (int i = 0; i < packetTotal; i++) {
            ...
            //만약 n개의 pkt을 보내야 되는데 마지막 pkt이 8byte라서 last가 0이고 이 pkt이 n번째라면
            //더 이상 보낼 pkt이 없음. 이 n 번째가 마지막 pkt이 되어야 함. 따라서 last pkt id 1로 set 
...
int read_can(){
        ...
        while (1) {
            memset(&frame,0,sizeof(frame));
                ...
                //printf("내가 보낸 id: %d, 이전에 받은 id: %d, 받은 id: %d\n",globalCan,pre,frame.can_id);
                //내가 write한 pkt이거나 loopback으로 인해 중복으로 받은 경우는 pass
                if(frame.can_id==globalCan||frame.can_id==pre) continue;
                //len이 2보다 큰 경우는 garbage pkt이므로 pass 
                if(frame.can_dlc>2) continue;
                // printf("id: %d\n",frame.can_id);
                pre=frame.can_id; 
                ...
                // }
                // printf("\n");
                memcpy(receiveMessage, frame.data, frame.can_dlc);
                receiveMessage[frame.can_dlc] = '\0'; 
